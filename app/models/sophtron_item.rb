...
class SophtronItem < ApplicationRecord
  ...
  def sync_status_summary
    ...
    if total_accounts == 0
      I18n.t("sophtron_items.sync_status.no_accounts")
    elsif unlinked_count == 0
      I18n.t("sophtron_items.sync_status.synced", count: linked_count)
    else
      I18n.t("sophtron_items.sync_status.synced_with_setup", linked: linked_count, unlinked: unlinked_count)
    end
  end
  ...
  def institution_summary
    ...
    when 0
      I18n.t("sophtron_items.institution_summary.none")
    when 1
      institutions.first["name"] || institutions.first["institution_name"] || I18n.t("sophtron_items.institution_summary.count", count: 1)
    else
      I18n.t("sophtron_items.institution_summary.count", count: institutions.count)
    end
  end
