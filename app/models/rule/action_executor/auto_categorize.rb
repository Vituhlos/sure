      ...
      if llm_provider
        ...
        suffix =
          if estimated_cost.nil?
            I18n.t("rules.action_executors.auto_categorize.cost_unavailable_suffix")
          else
            I18n.t(
              "rules.action_executors.auto_categorize.estimated_cost_suffix",
              cost: sprintf("%.4f", estimated_cost)
            )
          end
        "#{base_label}#{suffix}"
      else
