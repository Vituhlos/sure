...
class SureImport::Preflight
  Result = Struct.new(:errors, :warnings, :stats, keyword_init: true) do
    ...
    def error_message = valid? ? "" : ([ I18n.t("imports.sure_preflight.failed_header") ] + error_messages).join("\n")
  ...
  private
    ...
    def parse_records
      @content.each_line.with_index(1) do |line, line_number|
        ...
        unless record.is_a?(Hash)
          add_error(:invalid_ndjson_record, preflight_message(:record_must_be_object, line: line_number))
          next
        end
        ...
        if type.blank? || !record.key?("data")
          add_error(:invalid_ndjson_record, preflight_message(:record_must_include_type_and_data, line: line_number))
          next
        end

        @line_counts[type] += 1
        unless Family::DataImporter::SUPPORTED_TYPES.include?(type)
          add_error(:unsupported_record_type, preflight_message(:unsupported_record_type, line: line_number, type: type))
          next
        end

        unless data.is_a?(Hash)
          add_error(:invalid_ndjson_record, preflight_message(:data_must_be_object, line: line_number))
          next
        end
        ...
        if mapping_key && data["id"].present?
          track_source_id(mapping_key, data["id"], preflight_message(:record_location, line: line_number, type: type))
        end
        add_split_line_source_ids(data, line_number) if type == "Transaction"
      rescue JSON::ParserError
        add_error(:invalid_json, preflight_message(:invalid_json, line: line_number))
      end

      add_error(:no_data_rows, preflight_message(:no_data_rows)) if @rows_count.zero?
    end
    ...
    def add_split_line_source_ids(data, line_number)
      ...
      split_lines.each_with_index do |split_line, index|
        ...
        track_source_id(
          :transactions,
          split_line["id"],
          preflight_message(:split_line_location, line: line_number, index: index + 1)
        )
      end
    end
      ...
      TAXONOMY_TYPES.each do |type, association|
        ...
        @records[type].each do |record|
          ...
          add_error(
            ...
            preflight_message(:existing_taxonomy_collision, line: record[:line_number], type: type, name: name.inspect)
          )
        end
      end
    end
      ...
      TAXONOMY_TYPES.each_key do |type|
        ...
        grouped.each do |name, records|
          ...
          add_error(
            ...
            preflight_message(:duplicate_taxonomy_name, type: type, name: name.inspect, lines: lines)
          )
        end
      end
    end
      ...
      @source_id_locations.each do |mapping_key, ids|
        ids.each do |id, locations|
          ...
          add_error(
            ...
            preflight_message(
              :duplicate_source_id,
              resource: preflight_message("resource_types.#{mapping_key}"),
              id: id.inspect,
      ...
      @records.each do |type, records|
        ...
        records.each do |record|
          ...
          add_error(
            ...
            preflight_message(:missing_required_fields, line: record[:line_number], type: type, fields: missing.join(", "))
          )
        end
      end
    end
      ...
      @records["Account"].each do |record|
        ...
        unless accountable_class
          add_error(
            ...
            preflight_message(:invalid_accountable_type, line: record[:line_number], type: accountable_type.inspect)
          )
          next
        end
    ...
    def validate_split_lines
      @records["Transaction"].each do |record|
        ...
        unless split_lines.is_a?(Array)
          add_error(:invalid_split_lines, preflight_message(:split_lines_must_be_array, line: record[:line_number]))
          next
        end
        ...
        split_lines.each_with_index do |split_line, index|
          unless split_line.is_a?(Hash)
            add_error(
              :invalid_split_line,
              preflight_message(:split_line_must_be_object, line: record[:line_number], index: index + 1)
            )
          ...
          add_error(
            :missing_required_fields,
            preflight_message(:split_line_missing_amount, line: record[:line_number], index: index + 1)
          )
          complete_amounts = false
        end
    ...
    def validate_split_line_total(record, split_lines)
      ...
      add_error(
        ...
        preflight_message(
          :split_amount_mismatch,
          line: record[:line_number],
          expected: expected_amount.to_s("F"),
          actual: split_total.to_s("F")
        )
      )
    end
    ...
    def validate_reference(record, type, mapping_key, field, value)
      ...
      add_error(
        :missing_reference,
        preflight_message(:missing_reference, line: record[:line_number], type: type, field: field, value: value.inspect)
      )
    end
    ...
    def validate_split_line_references(record)
      ...
      Array(split_lines).each do |split_line|
        ...
        split_line_type = preflight_message(:transaction_split_line)
        validate_reference(record, split_line_type, :categories, "category_id", split_line["category_id"])
        validate_reference(record, split_line_type, :merchants, "merchant_id", split_line["merchant_id"])
        Array(split_line["tag_ids"]).each do |tag_id|
          validate_reference(record, split_line_type, :tags, "tag_ids", tag_id)
        end
      end
    end
    ...
    def preflight_message(key, **options)
      I18n.t("imports.sure_preflight.#{key}", **options)
    end
